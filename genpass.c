...
void convertCharToBit(long int n,const char pwd[])
{
    out = fopen("1.txt", "wb+");
    char ch;
    int i = n * 8;
...
int main() {
    ...
    while(1){
        ...
        printf("please input a user id:\n");
        scanf("%s",id);
        getchar();
        fprintf(fp,"%s",id);
        fprintf(fp," ");
        printf("please creat the corresponding user password:\n");
        scanf("%s",pwd);
        getchar();
        int i = 0;
        while(pwd[i]!='\0'&&i<PWD_SIZE)
            i++;
        int j;
        if(i%8==0)
            j=i;
            ...
            j= (i/8+1)*8;
        while(i<PWD_SIZE&&i<j){
            pwd[i]='*';
            i++;
        }

        long int n = i / 8;
        convertCharToBit(n,pwd);
        key64to48(key);
        ...
        char enc_pwd[PWD_SIZE*8];
        fgets(enc_pwd,i*8+1,fp1);
        fprintf(fp,"%s\n",enc_pwd);
        printf("press q to quit or press any other key to continue\n");
        if(getchar()=='q')
            break;
    }
    fclose(fp);
