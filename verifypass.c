...
void convertToBits(const int ch[])
{
    int value = 0;
    for (int i = 7; i >= 0; i--)
...
int main() {
    ...
    FILE* fp;
    fp = fopen("password.txt","r"); //here

    printf("please input your user id:\n");
    scanf("%s",id);
    getchar();
    while (!feof(fp)){
        char ret_whole[ID_SIZE+1+PWD_SIZE*8];
        memset(ret_whole,'\0',ID_SIZE+PWD_SIZE+1);
        char ret_id[ID_SIZE];
        memset(ret_id,'\0',ID_SIZE);
        char ret_pwd[PWD_SIZE*8];
        memset(ret_pwd,'\0',PWD_SIZE*8);

        fgets(ret_whole,ID_SIZE+2+PWD_SIZE*8,fp);
        int i = 0;
        while((ret_whole[i]!='\0')&&(ret_whole[i]!=' ')){
            ret_id[i]  = ret_whole[i];
            i++;
        }
        if(strcmp(ret_id,id)==0){
            ...
            int k = 0;
            while((ret_whole[j]!='\n')&&(ret_whole[i]!='\0')){
                ret_pwd[k]=ret_whole[j];
                j++;
                k++;
            }


            FILE* temp = fopen("2.txt","w+");
            fprintf(temp,"%s",ret_pwd);
            fclose(temp);
            ...
            char ret_dec_pwd[PWD_SIZE];
            memset(ret_dec_pwd,'\0',PWD_SIZE);

            temp = fopen("result.txt","r");
            fgets(ret_dec_pwd,PWD_SIZE+1,temp);
            fclose(temp);

            if(strcmp(pwd,ret_dec_pwd)==0){
